...
// Take is  connection
func Take() (*gorm.DB, *utils.Exception) {
	if currentCount >= poolsize {
		select {
		case ret := <-pool.bufpool:
			return ret, nil
		case <-time.After(time.Second * 2):
			return nil, utils.NewException(utils.Error, 1001, "time out")
		}
	} else {
		select {
		case ret := <-pool.bufpool:
		...
		default:
			...
			if err != nil {
				return nil, utils.NewException(utils.Error, 1002, "Create db connection error:"+err.Error())
			}
			currentCount++
			return db, nil
		}
